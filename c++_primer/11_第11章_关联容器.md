...
- 对于有序容器（map、multimap、set、multiset），关键字类型必须定义必须定义元素比较的方法
...
- 向 map 或 set 添加元素
  - 关联容器 insert 操作：
    ...
    - `c.insert(p, v)`:
      - 类似 insert(v)
      - v 是迭代器，用来提示从哪里==开始搜索==新元素应该存储的位置
...
- 无序容器的操作
  ...
  - 此外，还提供了无序容器特有的哈希管理操作
    ...
    - 备注：
      ...
      - 如果允许重复关键字，则所有关键字相同的元素也会在同一个桶中
      - 允许将不同关键字的元素映射到相同的桶，此时需要搜索桶中的元素来查找具体的关键字对应的元素
